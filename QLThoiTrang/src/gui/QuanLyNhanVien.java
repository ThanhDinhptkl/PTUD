...
public class QuanLyNhanVien extends JFrame implements ActionListener, MouseListener {
	...
	private void initComponents() {
		...
		btn_xoaTrang.setFont(new java.awt.Font("Segoe UI", 1, 24)); // NOI18N
		btn_xoaTrang.setIcon(new ImageIcon("D:\\Nhom18_PTUD_DoAn\\QLThoiTrang\\icon\\save.png")); // NOI18N
				...
				.addGroup(pn_RightLayout.createSequentialGroup()
						.addGroup(pn_RightLayout.createParallelGroup(GroupLayout.Alignment.LEADING)
								...
								.addGroup(pn_RightLayout.createSequentialGroup()
										.addContainerGap()
										.addGroup(pn_RightLayout.createParallelGroup(GroupLayout.Alignment.LEADING)
												.addComponent(btn_xoaTrang, GroupLayout.Alignment.TRAILING, GroupLayout.DEFAULT_SIZE, GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
		...
		pn_RightLayout.setVerticalGroup(
				...
				.addGroup(pn_RightLayout.createSequentialGroup()
						...
						.addComponent(btn_xoaTrang, GroupLayout.PREFERRED_SIZE, 60, GroupLayout.PREFERRED_SIZE)
						.addGap(101, 101, 101))
				);
	...
	private void deleteRowSelected() {
		...
		if (tbl_danhSachNV.getSelectedRowCount() == 0) {
			JOptionPane.showMessageDialog(null, "Bạn chưa chọn dòng để xóa");
		} else {

			//			lay ra danh sach xoa tren GUI
			int[] listIndex = tbl_danhSachNV.getSelectedRows();
			listMaNV += tbl_danhSachNV.getValueAt(listIndex[0], 0);
			for (int i = 1; i < tbl_danhSachNV.getSelectedRowCount(); i++) {
				listMaNV += ", " + tbl_danhSachNV.getValueAt(listIndex[i], 0);
			}
			...
			if (JOptionPane.showConfirmDialog(this, "Bạn có chắc chắn muốn xóa nhân viên " + listMaNV + " ?",
				...
				try {
					while (tbl_danhSachNV.getSelectedRowCount() > 0) {
						maNV = (String) tbl_danhSachNV.getValueAt(tbl_danhSachNV.getSelectedRow(), 0);
						if (!nhanVien_bus.xoa(maNV)) {
							JOptionPane.showMessageDialog(null, "Xóa thất bại, đã xảy ra lỗi");
							tbl_danhSachNV.clearSelection();
						} else {
							model.removeRow(tbl_danhSachNV.getSelectedRow());
						}
					}
				} catch (Exception e) {
	...
	private void fillForm(int r) {
		...
		txt_maNhanVien.setText(tbl_danhSachNV.getValueAt(r, 0).toString());
		txt_tenNhanVien.setText(tbl_danhSachNV.getValueAt(r, 1).toString());
		txt_sDT.setText(tbl_danhSachNV.getValueAt(r, 2).toString());
		txt_diaChi.setText(tbl_danhSachNV.getValueAt(r, 3).toString());
		String gioiTinh = tbl_danhSachNV.getValueAt(r, 4).toString();
		if(gioiTinh.equals("Nam")) {
			cmb_gioiTinh.setSelectedItem("Nam");
		}else {
			cmb_gioiTinh.setSelectedItem("Nữ");
		}
		txt_email.setText(tbl_danhSachNV.getValueAt(r, 5).toString());
		txt_ngaySinh.setText(tbl_danhSachNV.getValueAt(r, 6).toString());
	}
